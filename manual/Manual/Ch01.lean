...
`Design.lean` 은 방향을 뒤집는다. *그럴듯하지만 틀린 정의*를 주고, 그것이 무엇을 깨뜨리는지
증명하게 한다.

| 어디를 틀리게 했나 | 무엇이 깨지나 |
|---|---|
| 자유 변수 — 이항 논리 연산의 오른쪽을 빠뜨린다 | 일치 정리 |
| 의미 — 양화사에서 상태를 갱신하지 않는다 | 일치 정리 |
| 치환 — 결합 변수를 그대로 둔다 | 치환 정리 |
...
1장에는 채점되는 연습이 31 개 있다. 책 연습문제와 본문 명제가 섞여 있다.

| 갈래 | 개수 | 어디에 |
|---|---|---|
| 본문 명제 1.1 ~ 1.3 | 6 | `FreeVars.lean`, `Substitution.lean` |
| §1.3 건전성과 규칙 | 3 | `Validity.lean` |
| 책 연습 1.1 ~ 1.3 | 9 | `Ex.lean`, `Realizations.lean` |
| 책 연습 1.5 · 1.6 | 6 | `Ex/Summation.lean` |
| 정의를 왜 이렇게 쓰나 | 3 | `Design.lean` |
| 심화 트랙 | 4 | `Depth/` |

...
Reynolds가 "여기서는 다루지 않는다" 고 적고 지나가는 자리들이라 코드로 옮길 것이 없고,
그래서 이 문서에만 있다.
...
치환을 소박하게 쓰면 뜻이 달라지고, 그 사실을 다루는 데 §1.4의 절반이 들어간다.
...
트리를 받고, 상태를 받고, 정수를 낸다. 이 타입이 §1.2의 전부라고 해도 된다.
...
## 집합, 도메인, 범주론(category theory)은 무엇이 다른가

이 셋은 같은 질문에 내놓은 세 가지 답이 아니다. 먼저 값이 어디에 사는지를 정한다.
...
Reynolds는 §1.4 끝에서 결합 변수의 이름을 구체 구문에만 남기고, α-동치인 표기들을 같은
추상 구의 표현으로 보는 관점을 *고차 추상 구문(higher-order abstract syntax)*이라고
부른다. 현대 용례에서 HOAS는 더 구체적으로 메타언어의 함수와 결합자로 객체언어의 결합을
표현하는 기법을 가리킨다. 두 설명의 공통점은 결합 변수의 이름 자체를 추상 구문의 본질로
보지 않는다는 것이다.

이름을 다루는 방식은 크게 셋이다. 각 방식은 복잡성을 서로 다른 연산으로 옮긴다.
...
그러니 *α-변환이 뜻을 보존한다는 것은 언어 설계의 결과이지 공짜가 아니다.*

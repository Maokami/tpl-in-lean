...
theorem Ctx.fill_congr (C : Ctx V) {c c' : Comm V} (h : c.eval = c'.eval) :
  ...
  | iteL b C c₁ ih =>
      funext σ
      change (if ⟦b⟧ᵇ σ then (C.fill c).eval σ else c₁.eval σ)
          = (if ⟦b⟧ᵇ σ then (C.fill c').eval σ else c₁.eval σ)
      rw [ih]
  | iteR b c₀ C ih =>
      funext σ
      change (if ⟦b⟧ᵇ σ then c₀.eval σ else (C.fill c).eval σ)
          = (if ⟦b⟧ᵇ σ then c₀.eval σ else (C.fill c').eval σ)
      rw [ih]
  | wh b C ih => exact Comm.eval_wh_congr ih
  | «newvar» v e C ih =>
      funext σ
      change restore v σ ((C.fill c).eval (σ[v := ⟦e⟧ₑ σ]))
          = restore v σ ((C.fill c').eval (σ[v := ⟦e⟧ₑ σ]))
      rw [ih]
...
theorem obsEq_imp_eval_eq [Inhabited V] {c c' : Comm V} (h : ObsEq c c') : c.eval = c'.eval := by
  funext σ
  have hh := fun v => h .hole σ v
  simp only [observe, Ctx.fill] at hh
  rcases h1 : c.eval σ with _ | τ <;> rcases h2 : c'.eval σ with _ | τ'
  · rfl
  · have := hh default; rw [h1, h2] at this; simp at this
  · have := hh default; rw [h1, h2] at this; simp at this
  · have : τ = τ' := by
      funext v
      have := hh v

...
theorem forV4_eval_eq_forV3 (v w : V) (e₀ e₁ : IntExp V) (c : Comm V)
  ...
  have ha : ⟦e₀⟧ₑ (σ[w := ⟦e₁⟧ₑ σ]) = ⟦e₀⟧ₑ σ := by
    ...
    intro u hu
    exact State.subst_of_ne _ _ _ _ (fun h => hwe₀ (h ▸ hu))
  -- 안쪽 상태에서 제어 변수와 상계의 값.
  ...
  have e1 : (forV4 v w e₀ e₁ c).eval σ
      = restore w σ ((Comm.newvar v e₀
          (.ite (.cmp .le (.var v) (.var w)) (.seq (forWhileLt v w c) c) .skip)).eval
            (σ[w := ⟦e₁⟧ₑ σ])) := Comm.eval_isSemantics.2.2.2.2.2 _ _ _ _
  have e2 : (Comm.newvar v e₀
        (.ite (.cmp .le (.var v) (.var w)) (.seq (forWhileLt v w c) c) .skip)).eval
          (σ[w := ⟦e₁⟧ₑ σ])
      = restore v (σ[w := ⟦e₁⟧ₑ σ])
          ((Comm.ite (.cmp .le (.var v) (.var w)) (.seq (forWhileLt v w c) c) .skip).eval
            ((σ[w := ⟦e₁⟧ₑ σ])[v := ⟦e₀⟧ₑ (σ[w := ⟦e₁⟧ₑ σ])])) :=
    Comm.eval_isSemantics.2.2.2.2.2 _ _ _ _
  rw [e1, e2, ha]
  -- 조건은 `⟦e₀⟧ ≤ ⟦e₁⟧` 이다.
  have hcond : ⟦(.cmp .le (.var v) (.var w) : BoolExp V)⟧ᵇ
      ((σ[w := ⟦e₁⟧ₑ σ])[v := ⟦e₀⟧ₑ σ]) = decide (⟦e₀⟧ₑ σ ≤ ⟦e₁⟧ₑ σ) := by
    change decide (((σ[w := ⟦e₁⟧ₑ σ])[v := ⟦e₀⟧ₑ σ]) v
      ≤ ((σ[w := ⟦e₁⟧ₑ σ])[v := ⟦e₀⟧ₑ σ]) w) = _
    rw [hv₂, hw₂]
  change restore w σ (restore v (σ[w := ⟦e₁⟧ₑ σ])
      (if ⟦(.cmp .le (.var v) (.var w) : BoolExp V)⟧ᵇ _ then _ else _)) = _
  rw [hcond]
  by_cases hle : ⟦e₀⟧ₑ σ ≤ ⟦e₁⟧ₑ σ
  · -- 구간이 차 있다. 루프가 `(b-a).toNat` 번 돌고 본문이 한 번 더 돈다.
    rw [if_pos (by simp [hle])]
    have hm : (⟦e₁⟧ₑ σ - ⟦e₀⟧ₑ σ).toNat + 1 = (⟦e₁⟧ₑ σ - ⟦e₀⟧ₑ σ + 1).toNat := by omega
  ...
  · -- 구간이 비었다. 양쪽 다 본문을 안 돈다.
    ...
    have hzero : (⟦e₁⟧ₑ σ - ⟦e₀⟧ₑ σ + 1).toNat = 0 := by omega
    rw [hzero]
    rfl
-- ANCHOR_END: forV4Eq

...
theorem decrLoop_eval_of_halts :
  ...
  have key : ∀ (m : ℕ) (σ : State String), (σ "x" / 2).toNat = m → decrHalts σ →
    ...
    induction m with
    | zero =>
        -- 남은 바퀴가 0 이면 `x` 가 이미 0 이다. 조건이 거짓이라 그대로 끝난다.
        intro σ hm hh
        have h0 : σ "x" = 0 := by
          obtain ⟨hpos, heven⟩ := hh
          omega
        rw [unwindsDecr_eval σ, if_neg (by simp [h0]), State.subst_eq_self σ "x" h0]
    | succ n ih =>
        -- 한 바퀴 돌면 `x` 가 2 줄고 측도가 하나 준다.
        intro σ hm hh
        have h0 : σ "x" ≠ 0 := by
          obtain ⟨hpos, heven⟩ := hh
          omega
        rw [unwindsDecr_eval σ, if_pos h0, decr_step]
        have hnext : ((σ["x" := σ "x" - 2]) "x" / 2).toNat = n := by
          rw [State.subst_self]
          obtain ⟨hpos, heven⟩ := hh
          omega
        rw [ih (σ["x" := σ "x" - 2]) hnext (decrHalts_step hh h0), State.subst_subst]
  exact fun σ hh => key _ σ rfl hh

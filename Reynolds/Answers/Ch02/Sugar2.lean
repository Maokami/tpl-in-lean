...
public import Reynolds.Answers.Ch02.Sugar
-- `#guard`는 컴파일 시점에 계산한다 (AGENTS.md §10).
public meta import Reynolds.Answers.Ch02.Sugar
...
theorem forWhile_eq_fold (v w : V) (c : Comm V)
  ...
  induction m with
  | zero =>
      -- 횟수가 0 이면 `v` 가 이미 `w` 를 넘었다. 조건이 거짓이라 그대로 끝난다.
      intro σ hm
      have hgt : ¬ (σ v ≤ σ w) := by omega
      rw [whileEq σ, hBeval σ, if_neg (by simp [hgt])]
      rfl
  | succ n ih =>
      intro σ hm
      have hle : σ v ≤ σ w := by omega
      rw [whileEq σ, hBeval σ, if_pos (by simp [hle]), bodyEq σ, forFold]
      cases hc : c.eval σ with
      | none => rfl
      | some σ'' =>
          -- 본문은 `v` 도 `w` 도 건드리지 않았다 (명제 2.6(b)).
          have hv'' : σ'' v = σ v := Comm.eval_agree_outside_fa c σ σ'' hc v hv
          have hw'' : σ'' w = σ w := Comm.eval_agree_outside_fa c σ σ'' hc w hw
          -- 그래서 측도가 정확히 하나 준다.
          have hnext : ((σ''[v := σ'' v + 1]) w - (σ''[v := σ'' v + 1]) v + 1).toNat = n := by

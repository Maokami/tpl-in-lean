...
닫힌 정수 식은 정수 하나로 평가되지만, `x + 1` 같은 열린 식은 `x`의 값이 없으면
평가할 수 없다. 따라서 식의 표시는 정수 하나가 아니라, 상태를 받으면 정수를 내는 함수다.

```
IntExp V ──eval──▶ State V → Int
```

왼쪽은 구문 트리의 타입이고 오른쪽은 의미가 사는 반송자다. 이 둘을 구분해 두면
2장에서 명령의 의미 공간이 `State V → SigmaBot V`로 바뀌는 이유도 같은 타입 변화로 읽힌다.

의미 함수는 합성적(compositional)이다. 각 생성자의 의미가 직접 부분구의 의미만을 받아
정해진다는 뜻이다. Lean의 구조적 재귀는 이 구문 지향 방정식을 그대로 함수 정의로 만든다.
이 방정식이 의미 함수를 유일하게 정한다는 별도의 보편 성질은 `Depth/Algebra.lean`에서
준동형의 유일성으로 확인한다.
...
계산 가능한 `Bool` 평가기를 주려면 임의의 단언과 상태를 받아 참·거짓을 끝내 판정하는
절차가 필요하다. `⟦∀v. p⟧ σ = ∀ n : ℤ, ⟦p⟧ σ[v := n]`인 정수 양화와 곱셈을 포함한
표준 해석에는 그런 공통 판정 절차가 없다. 개별 단언이 결정 가능할 수 있다는 사실과
언어 전체의 진리 판정기가 존재한다는 주장은 다르다. `Prop`은 후자의 프로그램을 요구하지
않고도 뜻을 적을 수 있다.
...
따라서 여기서 `Prop`을 택한 것은 Reynolds의 `𝔹`을 실행 가능한 Lean `Bool`과 동일시하지
않기 위한 형식화 결정이다.

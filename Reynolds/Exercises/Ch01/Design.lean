...
자유 변수 집합이 왜 정확히 그만큼이어야 하는지가 그 장의 내용이다.
...
치환 정리(명제 1.3)는 "치환한 뒤 재기" 와 "재고 나서 상태를 갈아 끼우기" 가 같다고 말한다.
...
Lean 에서 이 정의는 아예 통과하지 않는다. 정지성을 따로 증명해서 `termination_by` 로
붙여야 한다 (구의 크기가 이름 바꾸기로 안 변한다는 사실이 필요하다).
그래서 잘못된 정의를 적어 두고 반례를 드는 방식으로는 보일 수가 없다 — 적을 수가 없으니까.
